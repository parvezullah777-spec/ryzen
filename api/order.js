...
async function handleList(req, res) {
  ...
  const from = req.query.from; // ISO date string, optional — powers Reports date filters
  const to = req.query.to;     // ISO date string, optional
  ...
  if (from) query = query.gte(COLS.createdAt, from);
  if (to) query = query.lte(COLS.createdAt, to);
