...
async function verifyFaceWithClaude(referenceImage, capturedImage) {
  ...
  const r = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 200,
      system:
        'You are a strict biometric face-verification agent guarding an admin login. ' +
        'You will be shown a reference photo of the authorized admin and a new snapshot taken at login. ' +
        'Compare specific facial features: eye shape and spacing, nose shape, jawline/face shape, eyebrow shape, ' +
        'and overall bone structure. Ignore lighting, camera angle, expression, hairstyle changes, glasses, ' +
        'and image quality differences \u2014 these are not identity signals. ' +
        'Only report match=true if you are highly confident (85+) these are the same individual. ' +
        'If the two photos show visibly different people \u2014 even if similar in age, gender, or general appearance \u2014 ' +
        'you must report match=false. When uncertain, default to match=false: it is far worse to let an ' +
        'unauthorized person in than to ask the real admin to retry. ' +
  ...
  try {
    ...
    return {
      // Belt-and-suspenders: even if the model reports match=true, don't treat it
      // as verified unless it also cleared the confidence bar. A borderline "true"
      // should not be enough to open the door.
      match: parsed.match === true && confidence >= MIN_CONFIDENCE,
      confidence,
